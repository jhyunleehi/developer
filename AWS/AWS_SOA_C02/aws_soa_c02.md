...
#### 추천 배포 정책
위의 설명을 바탕으로, "항상 전체 용량을 유지"하는 요구 사항을 충족하는 두 가지 배포 정책은 다음과 같습니다:

불변 (Immutable): 새로운 인스턴스를 생성하고, 테스트를 거쳐 성공적으로 작동하면 기존 인스턴스를 대체합니다. 이를 통해 기존 인스턴스의 용량을 유지하면서 새로운 인스턴스를 준비할 수 있습니다.

추가 배치로 롤링 (Additional Batch Rolling): 새로운 인스턴스를 추가 배치로 생성하고, 기존 인스턴스를 점진적으로 교체합니다. 이를 통해 전체 용량을 유지하면서 새로운 인스턴스로 교체할 수 있습니다.

따라서, **옵션 B (불변)**와 **옵션 E (추가 배치로 롤링)**이 이 요구 사항을 충족하는 배포 정책입니다.
...
#### 추천 조치
위의 설명을 바탕으로, "InsufficientInstanceCapacity" 오류를 해결하기 위한 두 가지 주요 조치는 다음과 같습니다:

* 다양한 가용성 영역에서 자동 크기 조정 그룹을 구성합니다 (옵션 B): 여러 가용성 영역에 걸쳐 Auto Scaling 그룹을 구성하여 특정 AZ의 용량 부족 문제를 완화할 수 있습니다.

* 인스턴스 서비스 할당량 증가를 요청합니다 (옵션 E): AWS Support에 문의하여 특정 인스턴스 유형의 서비스 할당량을 증가시켜 더 많은 인스턴스를 시작할 수 있는 용량을 확보할 수 있습니다.

이 두 가지 조치를 통해 InsufficientInstanceCapacity 오류를 효과적으로 해결할 수 있습니다.
...
#### 추천 조치
위의 설명을 바탕으로, "특정 태그가 추가된 EC2 인스턴스 그룹에 대한 접근을 제어"하기 위한 두 가지 주요 조치는 다음과 같습니다:

* EC2 인스턴스에 액세스해야 하는 사용자 또는 그룹에 IAM 정책을 연결합니다 (옵션 A): 사용자나 그룹별로 세밀한 접근 제어가 가능하며, 필요한 사용자나 그룹에 적절한 정책을 연결할 수 있습니다.

* Condition 요소에 지정된 태그가 있는 모든 EC2 인스턴스에 대한 액세스 권한을 부여하는 IAM 정책을 만듭니다 (옵션 E): 특정 태그가 있는 모든 EC2 인스턴스에 대한 접근 권한을 일괄적으로 부여할 수 있어 효율적입니다.

이 두 가지 조치를 통해 특정 태그가 추가된 EC2 인스턴스 그룹에 대한 접근을 효과적으로 제어할 수 있습니다.
...
#### 추천 솔루션
위의 설명을 바탕으로, 가장 적절한 솔루션은 옵션 A입니다. 이 옵션은 계정 A와 계정 B 간의 역할을 명확하게 분리하고, 서로 신뢰 관계를 설정하여 Lambda 함수가 S3 버킷에 접근할 수 있도록 합니다.

권장 조치:

계정 A: Lambda 실행 역할을 생성하고, 이 역할이 계정 B의 역할을 신뢰(trust)하도록 설정합니다.
계정 B: S3 버킷에 대한 액세스를 허용하는 역할을 생성합니다.
이렇게 하면 계정 A의 Lambda 함수가 계정 B의 S3 버킷에 안전하게 접근할 수 있습니다.
