...
프라이빗 서브넷의 Amazon EC2 인스턴스에 호스팅된 애플리케이션이 인터넷에 액세스할 수 없는 문제의 가능한 이유는 다음 두 가지입니다:

**C. NAT 게이트웨이가 지원되지 않는 가용성 영역에 있습니다.**  
NAT 게이트웨이는 특정 가용성 영역 내에서 작동합니다. 프라이빗 서브넷과 동일한 가용성 영역에 NAT 게이트웨이가 있어야 인터넷에 액세스할 수 있습니다.

**D. NAT 게이트웨이가 사용 가능한 상태가 아닙니다.**  
NAT 게이트웨이가 사용 불가능한 상태이거나 삭제된 경우, 프라이빗 서브넷의 EC2 인스턴스는 인터넷에 액세스할 수 없습니다. NAT 게이트웨이가 정상적으로 작동하는지 확인해야 합니다. 

이 두 가지 문제를 확인하고 해결하면 프라이빗 서브넷의 인스턴스가 인터넷에 정상적으로 액세스할 수 있을 것입니다.
...
주어진 시나리오에서, SysOps 관리자가 성능에 부정적인 영향을 미치지 않으면서 서비스 비용을 줄이기 위해 변경해야 할 인스턴스 유형은 **B. 대용량 메모리 최적화된 단일 온디맨드 인스턴스로 변경합니다**입니다.
...
이 경우 회사의 요구 사항을 충족하는 가장 운영 효율적인 솔루션은 **D. 트레일에서 CloudTrail 파일 무결성 기능을 활성화합니다. 보안 팀은 CloudTrail에서 생성한 다이제스트 파일을 사용하여 전달된 파일의 무결성을 확인할 수 있습니다**입니다.
...
단일 서브넷의 모든 인스턴스가 AWS Systems Manager에서 관리되지 않는 문제를 해결하기 위해 SysOps 관리자가 가장 먼저 해야 할 일은 **4. Systems Manager를 구성하여 인터페이스 VPC 엔드포인트를 사용하는 것**입니다.
...
회사가 Amazon S3 버킷에 대한 모든 액세스 시도를 기록하고, 삭제 이벤트에 대해 즉각적인 알림을 받아야 할 때 가장 적절한 솔루션은 **3. 감사 로그에는 Amazon CloudWatch Logs를 사용합니다. 알림 시스템에는 Amazon Simple Notification Service(Amazon SNS) 알림과 함께 Amazon CloudWatch 알람을 사용합니다.**입니다.
...
Amazon GuardDuty에서 식별한 외부 IP 주소로의 트래픽을 차단하는 가장 적절한 솔루션은 **3. 네트워크 ACL을 만듭니다. 외부 IP 주소로의 트래픽에 대한 아웃바운드 거부 규칙을 추가합니다.**입니다.
...
SysOps 관리자가 워크로드 비용을 최적화하고 있습니다. 워크로드는 여러 AWS 지역에서 실행 중이며 컴퓨터에 Amazon EC2 On-Demand 인스턴스와 함께 AWS Lambda를 사용하고 있습니다. 전체 사용량은 예측 가능합니다. 각 지역에서 사용되는 컴퓨터 양은 사용자의 위치에 따라 다릅니다.
...
SysOps 관리자가 여러 AWS 지역에서 실행 중인 워크로드를 최적화하려면 **A. 지난 30일간의 사용량을 기준으로 컴퓨터 절약 플랜을 구매하세요**가 가장 적합한 접근 방식입니다.
...
## 질문 #246
소프트웨어 회사가 Application Load Balancer(ALB) 뒤의 Amazon EC2 인스턴스에서 워크로드를 실행합니다. SysOps 관리자는 EC2 인스턴스에 대한 사용자 지정 상태 검사를 정의해야 합니다.
...
가장 운영 효율적인 솔루션은 **B. ALB에서 상태 검사를 구성하고 상태 검사 경로 설정이 올바른지 확인합니다**입니다.
...
## 질문 #248
한 회사가 회사의 AWS 워크로드와 관련된 리소스에 사용자 정의 태그를 적용합니다. 태그를 적용한 지 20일 후에 회사는 AWS Cost Explorer 콘솔에서 태그를 사용하여 뷰를 필터링할 수 없다는 것을 알게 됩니다.
...
이 문제의 이유는 **B. 회사에서 비용 할당을 위한 사용자 정의 태그를 활성화하지 않았습니다**입니다.
...
한 회사에 여러 AWS Lambda 함수를 사용하는 중요한 서버리스 애플리케이션이 있습니다. 각 Lambda 함수는 자체 Amazon CloudWatch Logs 로그 그룹에서 매일 1GB의 로그 데이터를 생성합니다. 회사의 보안 팀은 모든 로그 그룹에서 유형별로 그룹화된 애플리케이션 오류 수를 요청합니다.
...
이 요구 사항을 충족하기 위한 가장 적합한 솔루션은 **A. stats 명령과 count 함수를 사용하여 CloudWatch Logs Insights 쿼리를 수행합니다**입니다.
...
이 요구 사항을 충족하기 위한 최적의 솔루션은 **B. AWS Compute Optimizer를 활성화하고 Lambda 함수 권장 사항을 내보냅니다**입니다.
...
한 회사에서 AWS CloudFormation 템플릿을 사용하여 클라우드 인프라를 배포합니다. 회사의 모든 템플릿을 분석한 결과, 회사에서 여러 템플릿에 동일한 구성 요소를 선언한 것으로 나타났습니다. SysOps 관리자는 이러한 공통 구성 요소에 대한 자체 매개변수와 조건이 있는 전용 템플릿을 만들어야 합니다.
...
이 요구 사항을 충족하는 최적의 솔루션은 **C. CloudFormation 중첩 스택을 개발합니다**입니다.
...
이 요구 사항을 충족하는 최적의 솔루션은 **B. 스냅샷을 암호화하는 데 사용되는 AWS KMS 암호화 키에 모든 관련 계정에 대한 권한을 부여하도록 키 정책을 업데이트한 다음, 해당 계정과 스냅샷을 공유합니다**입니다.
...
SysOps 관리자가 VPC에서 Amazon S3 게이트웨이 엔드포인트를 구성합니다. VPC 내부의 프라이빗 서브넷에는 아웃바운드 인터넷 액세스가 없습니다. 사용자는 프라이빗 서브넷 중 하나에 있는 Amazon EC2 인스턴스에 로그인하고 동일한 AWS 리전의 Amazon S3 버킷에 파일을 업로드할 수 없습니다.
...
이 문제를 해결하는 최적의 솔루션은 **C. S3 게이트웨이 엔드포인트에 대한 S3 접두사 목록 대상 경로를 포함하도록 EC2 서브넷 경로 테이블을 업데이트합니다**입니다.
...
이 문제를 해결하는 가장 비용 효율적인 방법은 **C. Amazon S3 Transfer Acceleration을 사용**하고 **E. 다중 파트 업로드를 사용**하는 것입니다.
...
SysOps 관리자가 개발팀이 AWS에 애플리케이션을 배포하도록 돕고 있습니다. AWS CloudFormation 템플릿에는 Amazon Linux EC2 인스턴스, Amazon Aurora DB 클러스터, 90일마다 순환해야 하는 하드코딩된 데이터베이스 비밀번호가 포함되어 있습니다.
...
데이터베이스 비밀번호를 관리하는 가장 안전한 방법은 **A. AWS::SecretsManager::Secret 리소스를 GenerateSecretString 속성과 함께 사용하여 비밀번호를 자동으로 생성하고, AWS::SecretsManager::RotationSchedule 리소스를 사용하여 비밀번호의 로테이션 일정을 정의한 후, AWS Secrets Manager에서 비밀을 검색하여 데이터베이스에 액세스하도록 애플리케이션을 구성**하는 것입니다.
...
애플리케이션 A는 네트워크 로드 밸런서(NLB) 뒤의 Amazon EC2 인스턴스에서 실행됩니다. EC2 인스턴스는 자동 확장 그룹에 있으며 NLB와 연결된 동일한 서브넷에 있습니다. 온프레미스 환경의 다른 애플리케이션은 포트 8080에서 애플리케이션 A와 통신할 수 없습니다.
...
회사의 SysOps 관리자가 고가용성 환경을 유지 관리합니다. 이 환경에는 Amazon EC2 인스턴스와 Amazon RDS Multi-AZ 데이터베이스가 포함됩니다. EC2 인스턴스는 Application Load Balancer 뒤의 Auto Scaling 그룹에 있습니다.
...
회사의 VPC는 ​​AWS Site-to-Site VPN을 통해 온프레미스 데이터 센터에 연결됩니다. 회사는 VPC에 Amazon EC2 인스턴스가 있어야 example.com에 대한 DNS 쿼리를 데이터 센터의 DNS 서버로 보낼 수 있습니다.
...
SysOps 관리자는 데이터베이스 성능을 분석하는 업무를 맡았습니다. 데이터베이스는 단일 Amazon RDS DB 인스턴스에서 실행됩니다. SysOps 관리자는 트래픽이 가장 많은 시간에 읽기 트래픽 양으로 인해 데이터베이스의 리소스가 과도하게 사용된다는 것을 알게 되었습니다.
...
한 회사가 프로덕션 파일 서버를 AWS로 마이그레이션하고 있습니다. 파일 서버에 저장된 모든 데이터는 가용성 영역을 사용할 수 없게 되거나 시스템 유지 관리가 수행될 때에도 액세스 가능해야 합니다. 사용자는 SMB 프로토콜을 통해 파일 서버와 상호 작용할 수 있어야 합니다. 또한 사용자는 Windows ACL을 사용하여 파일 권한을 관리할 수 있어야 합니다.
...
SysOps 관리자는 Amazon EC2 인스턴스에 연결된 Amazon Elastic Block Store(Amazon EBS) 볼륨의 읽기 및 쓰기 메트릭을 기반으로 하는 알림을 만들어야 합니다. SysOps 관리자는 DiskReadBytes 메트릭과 DiskWriteBytes 메트릭에 대한 Amazon CloudWatch 알람을 만들고 활성화합니다.
...
한 회사가 최근 서버 인프라를 Amazon EC2 인스턴스로 이전했습니다. 이 회사는 Amazon CloudWatch 메트릭을 사용하여 인스턴스 메모리 사용률과 사용 가능한 디스크 공간을 추적하려고 합니다.
...
한 회사가 최근 기본 부팅 볼륨이 있는 Amazon EC2 인스턴스에 MySQL을 배포했습니다. 이 회사는 1.75TB 데이터베이스를 복원하려고 합니다. SysOps 관리자는 올바른 Amazon Elastic Block Store(Amazon EBS) 볼륨을 프로비저닝해야 합니다. 이 데이터베이스는 최대 10,000 IOPS의 읽기 성능이 필요하며 크기가 증가하지 않을 것으로 예상됩니다.
...
SysOps 관리자가 애플리케이션의 자동 확장 그룹에 Amazon EC2 인스턴스 플릿을 설정하고 있습니다. 플릿은 트래픽 버스트를 수용하기 위해 항상 50%의 CPU를 사용할 수 있어야 합니다. 부하는 주 7일, 오전 9시부터 오후 5시 사이에 상당히 증가합니다.
...
한 회사가 모든 기존 Amazon S3 버킷에 대한 서버 액세스 로깅을 켰습니다. 이 회사는 새 S3 버킷과 기존 S3 버킷의 로깅 설정을 모니터링하는 솔루션을 구현하려고 합니다. 이 솔루션은 로깅이 켜지지 않은 모든 S3 버킷을 수정해야 합니다.
...
한 회사가 Auto Scaling 그룹에서 Amazon EC2 On-Demand 인스턴스를 실행하고 있습니다. 인스턴스는 Amazon Simple Queue Service(Amazon SQS) 대기열의 메시지를 처리합니다. Auto Scaling 그룹은 대기열의 메시지 수에 따라 확장되도록 설정되어 있습니다. 메시지를 완전히 처리하는 데 최대 12시간이 걸릴 수 있습니다. SysOps 관리자는 메시지 처리 중에 인스턴스가 중단되지 않도록 해야 합니다.
...
한 회사가 AWS Organizations를 사용하여 AWS에서 계정 집합을 관리합니다. 회사의 보안 팀은 네이티브 AWS 서비스를 사용하여 Center for Internet Security(CIS) AWS Foundations Benchmark에 따라 모든 AWS 계정을 정기적으로 검사하려고 합니다.
...
한 회사가 현재 단일 가용성 영역의 VPC 내에서 인프라를 운영하고 있습니다. VPC는 ​​가상 사설 게이트웨이에 연결된 AWS 사이트 간 VPN 연결을 통해 회사의 온프레미스 데이터 센터에 연결됩니다. 온프레미스 경로 테이블은 모든 VPC 네트워크를 VPN 연결로 라우팅합니다. 두 환경 간의 통신이 올바르게 작동합니다. SysOps 관리자가 새 가용성 영역 내에 새 VPC 서브넷을 만들고 서브넷 내에 새 리소스를 배포했습니다. 그러나 새 리소스와 온프레미스 환경 간에 통신을 설정할 수 없습니다.
...
SysOps 관리자는 Ubuntu를 실행하는 수백 개의 Amazon EC2 인스턴스에 배포된 사용자 지정 애플리케이션에서 로그 파일의 내용을 수집해야 합니다. 로그 파일은 Amazon CloudWatch Logs에 저장해야 합니다.
...
SysOps 관리자는 AWS에서 애플리케이션에 대한 재해 복구(DR) 계획을 설계해야 합니다. 애플리케이션은 애플리케이션 로드 밸런서(ALB) 뒤의 Amazon EC2 인스턴스에서 실행됩니다. 인스턴스는 자동 확장 그룹에 있습니다. 애플리케이션은 Amazon Aurora PostgreSQL 데이터베이스를 사용합니다. 복구 시간 목표(RTO)와 복구 지점 목표(RPO)는 각각 15분입니다.
...
SysOps 관리자가 Amazon EC2에서 실행되는 간단한 공개 웹사이트를 만들고 있습니다. SysOps 관리자는 기존 퍼블릭 서브넷에 EC2 인스턴스를 만들고 인스턴스에 Elastic IP 주소를 할당했습니다. 그런 다음 SysOps 관리자는 인스턴스에 새 보안 그룹을 만들어 적용하여 0.0.0.0/0에서 들어오는 HTTP 트래픽을 허용했습니다. 마지막으로 SysOps 관리자는 새 네트워크 ACL을 만들어 서브넷에 적용하여 0.0.0.0/0에서 들어오는 HTTP 트래픽을 허용했습니다. 하지만 인터넷에서 웹사이트에 접근할 수 없습니다.
...
한 회사에 Amazon Elastic File System(Amazon EFS) 파일 시스템을 사용하는 애플리케이션이 있습니다. 애플리케이션 로직 오류와 관련된 최근 사고로 여러 파일이 손상되었습니다. 이 회사는 EFS 파일 시스템을 백업하고 복구하는 기능을 개선하고자 합니다. 이 회사는 개별 파일을 빠르게 복구할 수 있어야 합니다.
...
한 회사가 write-once, ready-many(WORM) 드라이브를 거버넌스 모드에서 S3 Object Lock이 구성된 Amazon S3 버킷으로 마이그레이션합니다. 마이그레이션하는 동안 회사는 불필요한 데이터를 S3 버킷에 복사합니다.

SysOps 관리자가 AWS CLI를 사용하여 S3 버킷에서 불필요한 데이터를 삭제하려고 시도합니다. 그러나 SysOps 관리자는 오류를 수신합니다. SysOps 관리자는

불필요한 데이터를 성공적으로 삭제하기 위해 어떤 단계 조합을 취해야 합니까? (두 가지를 선택하세요.)
A. 보관 기한을 늘립니다.
...
회사 웹사이트에는 AWS의 웹 계층과 데이터베이스 계층이 있습니다. 웹 계층은 두 개의 가용 영역에 걸쳐 Auto Scaling 그룹에서 실행되는 Amazon EC2 인스턴스로 구성됩니다. 데이터베이스 계층은 Amazon RDS for MySQL Multi-AZ DB 인스턴스에서 실행됩니다. 데이터베이스 서브넷 네트워크 ACL은 데이터베이스에 액세스해야 하는 웹 서브넷으로만 제한됩니다. 웹 서브넷은 기본 규칙이 있는 기본 네트워크 ACL을 사용합니다.
...
SysOps 관리자가 Auto Scaling 그룹에 있는 Amazon EC2 인스턴스에 애플리케이션을 배포할 준비를 하고 있습니다. 애플리케이션에는 종속성을 설치해야 합니다. 애플리케이션 업데이트는 매주 발행됩니다.
...
AWS CloudFormation 템플릿은 Amazon RDS 인스턴스를 생성합니다. 이 템플릿은 필요에 따라 개발 환경을 구축한 다음 환경이 더 이상 필요하지 않으면 스택을 삭제하는 데 사용됩니다. RDS에 저장된 데이터는 CloudFormation 스택이 삭제된 후에도 추가 사용을 위해 보관해야 합니다.
...
AnyCompany는 Example Corp를 인수하여 두 회사의 비즈니스 시스템을 통합하려고 시도하고 있습니다. AnyCompany의 IT 부서는 Example Corp의 IT 티켓팅 시스템과 통합해야 합니다.

SysOps 관리자는 AnyCompany 계정의 Amazon EC2 인스턴스에 대한 Amazon CloudWatch 알람을 사용하여 Example Corp의 티켓팅 시스템에서 새 티켓을 만드는 솔루션을 구현해야 합니다. 티켓팅 시스템은 새 티켓을 만들기 위한 HTTPS 엔드포인트를 제공합니다. 티켓팅 시스템은 다음 JSON 형식의 메시지를 허용합니다. CloudWatch 알람



에서 티켓을 만드는 어떤 접근 방식이 가장 적은 개발 시간으로 이러한 요구 사항을 충족할까요?
...
SysOps 관리자는 Amazon EC2 Auto Scaling 그룹에서 새로운 Amazon EC2 Spot Instances를 프로비저닝해야 합니다. Auto Scaling 그룹은 광범위한 인스턴스 유형을 사용합니다. 구성된 플릿은 시작된 인스턴스 수에 대해 가장 가용성이 높은 풀에서 가져와야 합니다.
...
SysOps 관리자가 eu-west-2 지역에서 사용자 지정 Amazon Machine Image(AMI)를 만들고 AMI를 사용하여 Amazon EC2 인스턴스를 시작합니다. SysOps 관리자는 동일한 AMI를 사용하여 다른 두 지역(us-east-1 및 us-east-2)에서 EC2 인스턴스를 시작해야 합니다.
...
회사는 AWS Organizations의 조직에 여러 계정을 가지고 있습니다. 회사는 조직의 관리 계정에서 멤버 계정으로 리소스 프로비저닝을 자동화해야 합니다.
...
한 회사에 Amazon S3 버킷을 만드는 AWS CloudFormation 템플릿이 있습니다. 사용자는 Active Directory 자격 증명으로 회사 AWS 계정에 인증하고 CloudFormation 템플릿을 배포하려고 시도합니다. 그러나 스택 생성이 실패합니다.
...
Amazon RDS for PostgreSQL DB 클러스터는 7일 보관 기간으로 자동 백업을 켭니다. SysOps 관리자는 원래 DB 클러스터에서 24시간 이내에 생성된 데이터를 사용하여 새 RDS DB 클러스터를 만들어야 합니다.
...
한 회사가 애플리케이션 로드 밸런서(ALB)를 사용하여 Amazon EC2에 호스팅된 글로벌 사용자 기반이 있는 웹사이트를 관리하고 있습니다. 웹 서버의 부하를 줄이기 위해 SysOps 관리자는 ALB를 원본으로 하는 Amazon CloudFront 배포를 구성합니다. 솔루션을 모니터링한 지 일주일 후, 관리자는 ALB에서 여전히 요청이 처리되고 있으며 웹 서버 부하에 변화가 없음을 알게 됩니다.
...
한 회사에 하이브리드 환경이 있습니다. 이 회사는 회사의 온프레미스 데이터 센터와 VPC에서 실행되는 워크로드 간에 AWS Direct Connect 연결을 설정했습니다. 이 회사는 AWS의 DNS에 Amazon Route 53을 사용합니다. 이 회사는 프라이빗 호스팅 영역을 사용하여 AWS에서 호스팅되는 일련의 서비스에 대한 DNS 이름을 관리합니다. 이 회사

는 온프레미스 서버에서 프라이빗 호스팅 영역의 DNS 확인을 위해 Route 53을 사용하기를 원합니다.
...
SysOps 관리자가 온프레미스 웹사이트의 고가용성에 대한 우려를 해결하기 위해 Amazon Route 53 DNS 옵션을 평가하고 있습니다. 이 웹사이트는 두 개의 서버로 구성되어 있습니다. 기본 활성 서버와 보조 수동 서버입니다. Route 53은 연관된 상태 검사에서 2xx 또는 3xx HTTP 코드가 반환되면 트래픽을 기본 서버로 라우팅해야 합니다. 다른 모든 트래픽은 보조 수동 서버로 전달해야 합니다. 장애 조치 레코드 유형, 세트 ID 및 라우팅 정책은 기본 서버와 보조 서버 모두에 대해 적절하게 설정되었습니다.
...
Amazon EC2 인스턴스는 Amazon Simple Queue Service(Amazon SQS) 대기열을 사용하는 애플리케이션을 실행하고 있습니다. SysOps 관리자는 애플리케이션이 SQS 대기열에서 메시지를 읽고, 쓰고, 삭제할 수 있는지 확인해야 합니다.
...
SysOps 관리자는 웹 애플리케이션을 호스팅하기 위해 Amazon S3 버킷을 구성해야 합니다. SysOps 관리자는 S3 버킷을 생성하고 웹 애플리케이션의 정적 파일을 S3 버킷에 복사했습니다.
...
한 회사가 AWS에서 웹 애플리케이션을 구축하고 있습니다. 이 회사는 www.example.com이라는 도메인 이름으로 Amazon CloudFront를 사용하고 있습니다. CloudFront로의 모든 트래픽은 전송 중에 암호화되어야 합니다. 이 회사는 이미 AWS Certificate Manager(ACM)에서 www.example.com에 대한 SSL 인증서를 프로비저닝했습니다.
...
한 회사가 3개의 가용성 영역에 있는 수백 개의 Amazon EC2 인스턴스에서 애플리케이션을 실행합니다. 애플리케이션은 퍼블릭 인터넷을 통해 타사 API를 호출합니다. SysOps 관리자는 타사가 애플리케이션의 트래픽을 허용할 수 있도록 타사에 정적 IP 주소 목록을 제공해야 합니다.
...
한 회사가 AWS Organizations를 사용하여 다중 계정 환경을 관리합니다. 이 회사는 기본 AWS 계정 중 하나에 Lifecycle: Production 태그가 표시된 모든 Amazon Elastic Block Store(Amazon EBS) 볼륨의 일일 증분 백업 생성을 자동화해야 합니다.
...
한 회사가 여러 가용성 영역에 걸쳐 Amazon EC2 인스턴스 플릿에 Windows 기반 파일 서버를 호스팅합니다. 현재 설정에서는 애플리케이션 서버가 EC2 플릿에서 동시에 파일에 액세스할 수 없습니다.
...
한 회사가 단일 VPC의 Amazon EC2 인스턴스에 애플리케이션을 배포했습니다. 이 회사는 EC2 인스턴스를 VPC의 프라이빗 서브넷에 배치했습니다.
...
SysOps 관리자는 웹사이트 페이지를 제공하는 Amazon CloudFront 배포의 캐싱을 관리합니다. SysOps 관리자는 개별 페이지의 TTL이 달라질 수 있도록 배포를 구성해야 합니다. 개별 페이지의 TTL은 배포에 설정된 최대 TLL 및 최소 TTL 내에 있어야 합니다.
...
한 회사에는 로컬 텔레비전에 광고가 게재된 후 트래픽이 급격히 증가하는 퍼블릭 웹 애플리케이션이 있습니다. 이 애플리케이션은 Auto Scaling 그룹에 있는 Amazon EC2 인스턴스에서 실행됩니다. Auto Scaling 그룹은 광고가 게재된 후 트래픽 급증에 대처하지 못합니다. 이 회사는 트래픽 급증 시 종종 100개의 EC2 인스턴스로 확장해야 합니다.
...
한 회사가 애플리케이션 로드 밸런서(ALB) 뒤의 Amazon EC2 온디맨드 인스턴스에서 내부 애플리케이션을 호스팅합니다. 인스턴스는 Amazon EC2 자동 확장 그룹에 있습니다. 직원들은 애플리케이션을 사용하여 잠재 고객에게 제품 가격을 제공합니다. 자동 확장 그룹은 동적 확장 정책으로 구성되고 인스턴스의 평균 CPU 사용률을 추적합니다.
...
개발자 팀은 여러 Amazon S3 버킷을 중앙 저장소로 사용하고 있습니다. 전 세계 사용자가 이러한 저장소에 대량의 파일을 업로드합니다. 개발팀의 애플리케이션은 나중에 이러한 파일을 처리합니다.
...
SysOps 관리자가 AWS Systems Manager Patch Manager를 사용하여 Amazon EC2 Windows 인스턴스 패치 프로세스를 자동화하려고 합니다. SysOps 관리자는 개발 인스턴스의 경우 릴리스 날짜로부터 2일 후에 패치가 자동 승인되도록 하려고 합니다. 또한 프로덕션 인스턴스의 경우 릴리스 날짜로부터 5일 후에 패치가 자동 승인되어야 합니다. 모든 인스턴스의 경우 유지 관리가 2시간 동안만 이루어져야 합니다.
...
SysOps 관리자는 10개의 AWS Lambda 함수에서 Amazon CloudWatch 로그를 분석하여 과거 오류를 파악해야 합니다. 로그는 JSON 형식이며 Amazon S3에 저장됩니다. 오류는 때때로 동일한 필드에 나타나지 않지만 모든 오류는 동일한 문자열 접두사로 시작합니다.
...
한 회사에서는 모든 Amazon EC2 인스턴스 로그를 Amazon CloudWatch Logs에 게시해야 한다는 정책을 가지고 있습니다. SysOps 관리자가 Amazon Linux 2를 실행하는 EC2 인스턴스의 문제를 해결하고 있습니다. EC2 인스턴스가 CloudWatch Logs에 로그를 게시하지 않습니다. Amazon CloudWatch 에이전트가 EC2 인스턴스에서 실행 중이고 에이전트 구성 파일이 올바릅니다.
...
한 회사가 Amazon EC2 인스턴스에서 워크로드를 실행합니다. 워크로드에는 자주 변경되는 데이터가 포함된 임시 캐시가 필요합니다. 워크로드는 인스턴스 재시작 시 캐시를 유지할 필요가 없습니다.
...
한 회사가 AWS Organizations에서 조직 전반에 걸쳐 여러 워크로드를 실행합니다. 회사의 재무 팀은 비용 변화를 추적하고 자세한 비용 지표를 제공하기 위한 자세한 대시보드가 ​​필요합니다. 재무 팀은 매시간 세부적으로 추세를 추적해야 합니다.
...
회사에는 주 7일, 하루 24시간 실행해야 하는 핵심 애플리케이션이 있습니다. 이 애플리케이션은 Amazon EC2, AWS Fargate, AWS Lambda를 사용합니다. 이 회사는 여러 AWS 지역에서 운영 체제를 조합하여 사용합니다.

이 회사는 변경에 유연성을 제공하는 가격 책정 모델을 고수하면서 비용 절감을 극대화해야 합니다. 이
...
SysOps 관리자는 개발자가 Amazon EC2 인스턴스에서 CPU 집약적 작업을 실행하는 AWS 계정을 관리합니다. 작업을 완료하는 데 며칠이 걸릴 수 있으며 때로는 여러 번 반복해야 할 수도 있습니다. 개발자는 작업이 완료되면 인스턴스를 종료하는 것을 종종 잊어버립니다.
...
한 회사에 Amazon EC2를 사용하고자 하는 여러 사업부가 있습니다. 이 회사는 모든 사업부가 승인된 EC2 인스턴스 구성만 사용하여 EC2 인스턴스를 프로비저닝하도록 요구하고자 합니다.
...
SysOps 관리자는 AWS CloudFormation 템플릿에서 생성된 Amazon RDS 데이터베이스의 자격 증명을 보호해야 합니다. 솔루션은 자격 증명을 암호화해야 하며 자동 로테이션을 지원해야 합니다.
...
한 회사가 AWS 내에서 Amazon EC2와 Amazon RDS에 대한 지출을 추적하고자 합니다. 이 회사는 AWS 계정의 리소스에 대해 더 엄격한 태그 지정 요구 사항을 구현하기로 결정했습니다. SysOps 관리자는 모든 비준수 리소스를 식별해야 합니다.
...
한 회사에서 새로운 Amazon FSx for Windows File Server 파일 시스템을 만듭니다. 비용을 관리하기 위해 회사는 최소한의 성장 여지를 두고 파일 시스템의 스토리지 용량을 구성합니다.

회사는 파일 시스템이 있는 동일한 AWS 계정에 Amazon Simple Notification Service(Amazon SNS) 토픽을 만듭니다. 회사는 SysOps 관리자의 이메일 주소를 SNS 토픽에 구독합니다. SysOps 관리자는 파일 시스템의 사용 가능한 공간이 100GB 미만일 때 이메일 알림을 받아야 합니다. SysOps 관리자

는 이 요구 사항을 충족하기 위해 어떤 단계 조합을 취해야 합니까? (두 가지를 선택하세요.)
...
한 회사가 EC2 인스턴스 중에 비생산 Amazon EC2 인스턴스를 중지하기로 결정했습니다. 회사의 IT 관리자는 밤에 비생산이라는 환경 유형 태그 값을 가진 EC2 인스턴스가 시작될 때마다 거의 실시간으로 알림을 받아야 합니다.
...
회사의 SysOps 관리자는 단일 AWS 계정에서 실행되는 Windows Amazon EC2 인스턴스의 플릿을 관리합니다. 인스턴스에는 "OS" 키와 "Windows" 값이 포함된 태그가 있습니다. 회사는 AWS Systems Manager를 사용하여 인스턴스에 패치를 적용합니다.
...
한 회사에서 Amazon EC2 인스턴스에서 실행되는 레거시 소프트웨어에 문제가 있습니다. EC2 인스턴스의 총 CPU 사용률이 80%를 초과하면 오류가 발생합니다. 소프트웨어를 다시 작성하는 동안 단기 솔루션이 필요합니다. SysOps 관리자는 CPU 사용률이 80%를 초과하면 인스턴스를 다시 시작하는 솔루션을 만드는 작업을 맡았습니다.
...
SysOps 관리자가 VPC의 프라이빗 서브넷에서 Amazon EC2 인스턴스를 시작합니다. SysOps 관리자가 EC2 인스턴스의 명령줄에서 curl 명령을 시도하면 SysOps 관리자가 https:www.example.com에 연결할 수 없습니다.
...
애플리케이션 팀은 하나의 Aurora Replica가 있는 Amazon Aurora MySQL DB 클러스터를 사용합니다. 애플리케이션 팀은 사용자 연결이 200을 초과하면 애플리케이션 읽기 성능이 저하된다는 것을 알아차렸습니다. 사용자 연결 수는 일반적으로 180으로 일정하며, 가끔 200개 이상의 연결로 갑자기 증가합니다. 애플리케이션 팀은 사용자 수요가 증가하거나 감소함에 따라 애플리케이션이 자동으로 확장되기를 원합니다.
...
한 회사가 Amazon Elastic Block Store(Amazon EBS)가 지원하는 Amazon EC2 인스턴스에 프로덕션 데이터베이스를 호스팅합니다. 연간 재해 복구 작업의 일환으로 회사는 최근 EBS 스냅샷을 두 번째 가용성 영역의 새 EC2 인스턴스로 복원해야 합니다.
...
SysOps 관리자가 레거시 CPU 중심 애플리케이션을 담당하고 있습니다. 애플리케이션은 수직으로만 확장할 수 있습니다. 현재 애플리케이션은 단일 t3.large Amazon EC2 인스턴스에 배포되어 있습니다. 시스템은 몇 분 후에 90%의 CPU 사용률과 상당한 성능 지연을 보이고 있습니다.
...
• EC2 인스턴스의 아웃바운드 보안 그룹에는 포트 443에서 CIDR 0.0.0.0/0으로의 아웃바운드라는 규칙이 하나 있습니다. • EC2 인스턴스
의 인바운드 보안 그룹은 사용자 IP 주소에서 포트 22와 443을 허용합니다.
...
회사의 재무 부서는 AWS 계정에서 각 프로젝트의 비용 세부 정보를 확인해야 합니다. SysOps 관리자는 Cost Explorer에서 각 프로젝트의 비용을 확인하는 데 필요한 초기 구성을 수행해야 합니다.
...
SysOps 관리자가 Application Load Balancer(ALB) 뒤의 Amazon EC2 인스턴스에서 실행되는 웹 애플리케이션을 관리하고 있습니다. 인스턴스는 EC2 Auto Scaling 그룹에서 실행됩니다. 관리자는 ALB와 연관된 모든 대상 인스턴스가 비정상일 때 알람을 설정하려고 합니다.
...
한 회사에서 AWS Organizations를 사용하여 다중 계정 환경을 관리합니다. 이 조직에는 보안을 위한 전담 계정과 로깅을 위한 전담 계정이 있습니다. SysOps 관리자는 모든 계정의 리소스 메트릭이 표준 정의된 임계값을 넘을 때 알림을 제공하는 중앙 집중식 솔루션을 구현해야 합니다.
...
한 회사에는 인터넷을 통해 외부 소스에서 데이터 세트를 검색하기 위해 예약된 AWS Lambda 함수를 사용하는 애플리케이션이 있습니다. 이 함수는 VPC와 연결되어 있지 않습니다. 이 회사는 Lambda 함수가 검색하는 정보를 프라이빗 서브넷의 Amazon RDS DB 인스턴스에 저장하도록 애플리케이션을 수정하고 있습니다. VPC에는 퍼블릭 서브넷 2개와 프라이빗 서브넷 2개가 있습니다.
...
## 한 회사가 Amazon RDS for MySQL db.m6g.xlarge(일반 용도) 표준 DB 인스턴스의 Multi-AZ 배포를 사용하는 프로덕션 워크로드를 실행하고 있습니다. 사용자들은 "연결이 너무 많음" 오류가 자주 발생한다고 보고합니다. SysOps 관리자는 데이터베이스의 연결 수가 많다는 것을 관찰합니다. SysOps 관리자는

코드 변경을 최소한으로 유지하면서 이 문제를 해결해야 합니다.
...
한 회사에 개발 환경에서 리소스 집약적 애플리케이션을 실행하는 여러 Amazon EC2 인스턴스가 있습니다. SysOps 관리자가 사용되지 않을 때 이러한 EC2 인스턴스를 중지하는 솔루션을 구현하고 있습니다.
...
한 회사에는 MySQL을 실행하는 Amazon EC2 인스턴스로 구성된 데이터베이스 계층이 있는 웹 애플리케이션이 있습니다. SysOps 관리자는 잠재적인 데이터 손실과 데이터베이스 장애 발생 시 복구하는 데 필요한 시간을 최소화해야 합니다.
...
한 회사가 AWS CloudFormation을 사용하여 AWS에서 Amazon EC2 인스턴스 스택을 관리합니다. SysOps 관리자는 누군가가 스택을 삭제하더라도 인스턴스와 모든 인스턴스 데이터를 보관해야 합니다.
...
Accompany는 실행 중인 Amazon EC2 인스턴스 수를 모니터링하려고 합니다. 또한 인스턴스 수가 특정 임계값에 도달하면 서비스 할당량 증가를 자동화하려고 합니다.
...
SysOps 관리자는 단일 프로덕션 AWS 계정에 배포된 50개 이상의 Amazon EC2 인스턴스를 담당합니다. EC2 인스턴스는 여러 가지 운영 체제를 실행합니다. 회사의 표준에 따라 패치는 한 달에 한 번 이상 완료해야 합니다.
...
회사에 여러 AWS 계정이 있습니다. 회사는 프로덕션 계정에 대한 조직 단위(OU)와 개발 계정에 대한 또 다른 OU가 있는 AWS Organizations를 사용합니다. 회사 정책에 따르면 개발자는 프로덕션 계정에서 승인된 AWS 서비스만 사용할 수 있습니다.
...
한 회사에는 매분 여러 번 트랜잭션 요청을 처리하는 애플리케이션이 있습니다. 이 애플리케이션은 단일 Amazon RDS DB 인스턴스에 트랜잭션 데이터를 씁니다. 회사가 더 많은 트랜잭션을 처리하기 시작하면서 회사는 재해 복구(DR)를 위한 장애 조치 솔루션이 없다는 점에 대해 우려하게 됩니다. 이 회사는 커밋된 트랜잭션을 잃지 않고 DB 인스턴스가 자동으로 장애 조치되어야 합니다.
...
ASysOps 관리자는 기본 설정을 사용하여 간단한 확장 자동 확장 그룹에서 애플리케이션 로드 밸런서(ALB) 뒤의 Amazon EC2 인스턴스에서 실행되도록 애플리케이션을 구성합니다. 자동 확장 그룹은 확장을 위해 RequestCountPerTarget 메트릭을 사용하도록 구성됩니다. SysOps 관리자는 RequestCountPerTarget 메트릭이 180초 동안 지정된 한도를 두 번 초과했다는 것을 알아챘습니다.
...
한 회사에는 ALB(Application Load Balancer) 뒤의 Amazon EC2 인스턴스에서 실행되는 보안 웹사이트가 있습니다. AWS Certificate Manager(ACM)의 SSL 인증서가 ALB에 사용됩니다. 레거시 웹 브라우저를 사용하는 사용자는 웹사이트에서 문제를 겪고 있습니다.
...
한 회사가 Amazon EC2 인스턴스에서 인터넷 웹 애플리케이션을 호스팅합니다. 이 회사는 애플리케이션을 새로운 AWS Lambda 함수로 대체하고 있습니다. 전환 기간 동안 이 회사는 일부 트래픽을 레거시 애플리케이션으로, 일부 트래픽을 새로운 Lambda 함수로 라우팅해야 합니다. 이 회사는 라우팅을 결정하기 위해 요청의 URL 경로를 사용해야 합니다.
...
SysOps 관리자가 퍼블릭 서브넷에서 Amazon EC2 Linux 인스턴스를 시작합니다. 인스턴스가 실행 중일 때 SysOps 관리자는 퍼블릭 IP 주소를 얻고 인스턴스에 원격으로 여러 번 연결을 시도합니다. 그러나 SysOps 관리자는 항상 시간 초과 오류를 받습니다.
...
한 회사가 Amazon EC2 인스턴스에서 고성능 컴퓨팅(HPC) 애플리케이션을 실행합니다. 이 회사는 이 아키텍처를 두 개 이상의 EC2 인스턴스로 확장해야 합니다. EC2 인스턴스는 애플리케이션을 지원하기 위해 낮은 대기 시간으로 고속으로 서로 통신해야 합니다.
...
개발자가 객체가 Amazon S3 버킷에 들어갈 때 실행되는 AWS Lambda 함수를 만듭니다. 이 함수는 객체를 다시 포맷하고 객체를 다시 S3 버킷에 넣습니다. 테스트하는 동안 개발자는 재귀적 호출 루프를 알아차립니다. 개발자는 SysOps 관리자에게 재귀적 호출을 즉시 중지해 달라고 요청합니다.
...
한 회사에는 us-west-2 지역의 Application Load Balancer(ALB) 뒤에서 실행되는 애플리케이션이 있습니다. Amazon Route 53 레코드 세트에는 us-west-2의 ALB를 참조하고 간단한 라우팅 정책을 사용하는 app.anycompany.com에 대한 별칭 레코드가 포함되어 있습니다. 이 애플리케이션은 전 세계 다른 지역에서 온 사용자가 증가하고 있습니다. 이러한 사용자는 높은 지연 시간을 겪고 있습니다.
...
한 회사가 동일한 AWS 리전의 50개 Amazon S3 버킷에 파일을 저장합니다. 이 회사는 Amazon EC2 인스턴스에서 개인 연결을 통해 S3 버킷에 안전하게 연결하려고 합니다. 이 회사는 추가 비용이 발생하지 않는 솔루션이 필요합니다.
...
회사의 보안 정책에 따르면 SSH 및 ROP를 통한 Amazon EC2 인스턴스 연결이 허용되지 않습니다. 액세스가 필요한 경우 권한이 있는 직원은 AWS Systems Manager Session Manager를 사용하여 인스턴스에 연결할 수 있습니다.
...
D. VPC의 DHCP 옵션 세트를 수정합니다. VPN 섹션에 IPsec 터널을 추가합니다.



## 질문 #349

한 회사가 Amazon Aurora 단일 노드 DB 클러스터에 프로덕션 MySQL 데이터베이스를 호스팅합니다. 이 데이터베이스는 보고 목적으로 대량으로 쿼리됩니다. DB 클러스터는 높은 CPU 사용률과 최대 연결 오류로 인해 성능 저하 기간을 겪고 있습니다. SysOps 관리자는 데이터베이스의 안정성을 개선해야 합니다.
...
어떤 회사가 사용자가 www example com이라는 이름으로 액세스하는 웹 애플리케이션을 실행합니다. 이 회사는 Amazon Route 53을 사용하여 example.com이라는 도메인 이름을 관리합니다. 이 회사는 애플리케이션 앞에 Amazon CloudFront 배포를 생성했고 www.example.com이 CloudFront를 통해 애플리케이션에 액세스하기를 원합니다.
...
한 회사가 AWS Organizations에서 여러 AWS 계정을 관리하고 있습니다. 이 회사는 AWS 환경의 내부 보안을 검토하고 있습니다. 이 회사의 보안 관리자는 자체 AWS 계정을 가지고 있으며 개발자 AWS 계정의 VPC 구성을 검토하려고 합니다.
...
한 회사가 Amazon EC2 Auto Scaling 그룹에 있는 Amazon EC2 인스턴스에서 애플리케이션을 실행합니다. 확장 작업은 오래 실행되는 부팅 스크립트로 인해 완료되는 데 오랜 시간이 걸립니다. SysOps 관리자는 Auto Scaling 그룹을 과도하게 프로비저닝하지 않고도 확장 작업에 필요한 시간을 줄이는 솔루션을 구현해야 합니다.
...
한 회사가 SysOps 관리자에게 4개의 추가 AWS 리전에서 애플리케이션에 대한 추가 환경을 프로비저닝해 달라고 요청했습니다. 이 애플리케이션은 us-east-1 리전에서 100개가 넘는 Amazon C2 인스턴스에서 완전히 구성된 Amazon Machine Images(AMI)를 사용하여 실행 중입니다. 이 회사에는 us-east-1에 리소스를 배포하기 위한 AWS CloudFormation 템플릿이 있습니다.
...
한 회사가 많은 수의 Amazon EC2 인스턴스에서 애플리케이션을 실행합니다. SysOps 관리자는 EC2 인스턴스 상태가 변경될 때마다 운영 팀에 알리는 솔루션을 구현해야 합니다.
...
한 회사가 레거시 온프레미스 웹 애플리케이션을 Amazon EC2 인스턴스로 마이그레이션했습니다. 웹 애플리케이션에는 트래픽을 수락하고 요청을 처리하기 위한 단일 정적 공용 IP 주소가 필요합니다. 최종 사용자는 example.com 도메인을 통해 웹 애플리케이션에 접속할 수 있어야 합니다. SysOps 관리자는 최소한의 노력으로 웹 애플리케이션을 유지 관리하는 솔루션을 구현해야 합니다.
...
한 회사가 데이터에 Amazon DynamoDB 테이블을 사용하고 있습니다. SysOps 관리자는 재해 복구를 위해 다른 AWS 리전으로 테이블 복제를 구성해야 합니다.
...
회사에 www.example.com에 대한 기존 퍼블릭 웹 애플리케이션이 있습니다. 애플리케이션 로드 밸런서(ALB)는 단일 HTTP 80 리스너로 구성되어 있습니다. SysOps 관리자는 www.example.com에 대한 모든 웹 요청이 클라이언트와 ALB 간에 암호화되도록 해야 합니다.
...
한 회사가 Amazon EC2 인스턴스에서 전체 애플리케이션 제품군을 실행합니다. 이 회사는 애플리케이션을 컨테이너와 AWS Fargate로 옮길 계획입니다. 이 회사는 6개월 이내에 EC2 인스턴스를 폐기하고 Fargate만 사용할 계획입니다. 이 회사는 향후 Fargate 비용을 추산할 수 있었습니다.
...
제조 회사는 Amazon RDS DB 인스턴스를 사용하여 모든 재고 품목의 재고를 저장합니다. 이 회사는 품목을 추가, 업데이트 및 삭제하기 위해 데이터베이스와 상호 작용하는 여러 AWS Lambda 함수를 유지 관리합니다. Lambda 함수는 하드코딩된 자격 증명을 사용하여 데이터베이스에 연결합니다.
...
개발자가 Application Load Balancer(ALB) 뒤의 Amazon EC2 인스턴스에서 실행되는 웹 애플리케이션을 만듭니다. 인스턴스는 Auto Scaling 그룹에 있습니다. 개발자는 배포를 검토하고 애플리케이션으로의 의심스러운 트래픽을 알아차립니다. 트래픽은 악성이며 단일 공용 IP 주소에서 발생합니다. SysOps 관리자는 공용 IP 주소를 차단해야 합니다.
...
SysOps 관리자가 AWS에서 리소스를 생성하고 있습니다. Amazon EC2 인스턴스의 자동 확장 그룹을 정의하는 CloudFbrmation 템플릿입니다. 자동 확장 그룹 시작 템플릿은 사용자 데이터 스크립트를 사용하여 각 EC2 인스턴스를 프로비저닝합니다. 오류로 인해 자동 확장 그룹 리소스 생성이 실패합니다. 대기 조건이 필요한 수의 신호를 수신하지 못합니다.
...
한 회사가 두 애플리케이션을 연결하려고 합니다. 한 애플리케이션은 호스트 이름이 host1.onprem private인 온프레미스 데이터 센터에서 실행됩니다. 다른 애플리케이션은 호스트 이름이 host1.awscloud private인 Amazon EC2 인스턴스에서 실행됩니다. 온프레미스 네트워크와 AWS 사이에 AWS 사이트 간 VPN 연결이 있습니다.
...
한 회사가 여러 AWS 리전에 애플리케이션 인스턴스와 관련 인프라를 배포해야 합니다. 이 회사는 이 목표를 달성하기 위해 단일 AWS CloudFormation 템플릿을 사용하려고 합니다. 이 회사는 AWS Organizations를 사용하고 중앙 관리 계정에서 이 템플릿을 관리하고 실행하려고 합니다.
...
회사의 SysOps 관리자는 Windows 기반 워크로드와 Linux 기반 워크로드를 실행하는 수백 개의 Amazon EC2 인스턴스 플릿을 관리합니다. 각 EC2 인스턴스에는 운영 체제를 식별하는 태그가 있습니다. 모든 EC2 인스턴스는 AWS Systems Manager Session Manager를 실행합니다.
...
한 회사에는 수천 개의 경보 시스템에서 알림을 수집하는 애플리케이션이 있습니다. 알림에는 경보 알림과 정보 알림이 포함됩니다. 정보 알림에는 시스템 무장 프로세스, 무장 해제 프로세스 및 센서 상태가 포함됩니다.
...
SysOps 관리자는 여러 AWS 리전에 애플리케이션을 배포해야 합니다. SysOps 관리자는 사용자를 가장 낮은 지연 시간을 가진 리전으로 라우팅하는 솔루션을 구현해야 합니다. 장애가 발생할 경우 솔루션은 애플리케이션의 건강한 인스턴스가 있는 리전으로 요청을 자동으로 라우팅해야 합니다. 회사에는 장애 조치 시간이 가장 짧은 솔루션이 필요합니다.
...
한 회사가 애플리케이션 로드 밸런서 뒤의 Amazon EC2 인스턴스에서 애플리케이션을 실행합니다. EC2 인스턴스는 자동 확장 그룹에 있습니다. 애플리케이션이 때때로 느리고 응답하지 않습니다. Amazon CloudWatch 메트릭은 일부 EC2 인스턴스가 높은 CPU 부하를 겪고 있음을 보여줍니다.
...
## 한 회사에 ap-southeast-2 지역에 호스팅된 암호화된 Amazon S3 버킷이 있습니다. eu-west-2 지역의 사용자는 인터넷을 통해 S3 버킷에 액세스합니다. eu-west-2 지역의 사용자는 대용량 파일에 대해 S3 버킷으로의 전송과 S3 버킷에서의 전송이 더 빨라야 합니다.
...
한 회사에 대규모 온프레미스 테이프 백업 솔루션이 있습니다. 이 회사는 AWS Storage Gateway를 사용하기 시작했습니다. 이 회사는 기존 온프레미스 하드웨어를 대체하기 위해 Tape Gateway를 만들었습니다. 이 회사의 백업 엔지니어는 AWS에 쓰기로 되어 있던 일부 백업 작업이 "공간 부족" 오류로 인해 실행되지 않는다는 것을 알아챘습니다.
...
SysOps 관리자가 회사의 Amazon S3 버킷을 관리합니다. SysOps 관리자는 회사의 AWS 계정에서 S3 버킷에 5GB의 불완전한 멀티파트 업로드를 식별했습니다. SysOps 관리자는 S3 버킷에서 불완전한 멀티파트 업로드 객체의 수를 줄여야 합니다.
...
SysOps 관리자는 AWS Organizations 구조에서 많은 AWS 멤버 계정에 대한 정책을 관리합니다. 다른 팀의 관리자는 멤버 계정의 계정 루트 사용자 자격 증명에 액세스할 수 있습니다. SysOps 관리자는 관리자를 포함한 모든 팀이 Amazon DynamoDB를 사용하지 못하도록 해야 합니다. 솔루션은 팀이 다른 AWS 서비스에 액세스하는 기능에 영향을 미쳐서는 안 됩니다.
...
회사에는 필요한 것보다 더 많은 디스크 성능 용량이 있는 Amazon EC2 인스턴스를 배포하는 사용자가 있습니다. SysOps 관리자는 인스턴스와 연결된 모든 Amazon Elastic Block Store(Amazon EBS) 볼륨을 검토하고 IOPS 및 처리량을 기반으로 비용 최적화 권장 사항을 만들어야 합니다.
...
SysOps 관리자는 노드 간에 파일 시스템을 공유해야 하는 많은 Windows Amazon EC2 인스턴스를 가지고 있습니다. SysOps 관리자는 Amazon Elastic File System(Amazon EFS) 파일 공유를 만듭니다. 파일 공유를 만든 후 SysOps 관리자는 파일 공유를 EC2 인스턴스에 마운트하는 데 문제가 있습니다.
...
한 회사에 다중 계정 환경이 있습니다. 계정 A에는 Amazon EC2 인스턴스에 호스팅된 프로덕션 애플리케이션이 있습니다. 이 애플리케이션은 계정 B에 호스팅된 Amazon DynamoDB 테이블의 데이터를 쿼리해야 합니다.
...
한 회사가 마이크로서비스 기반 애플리케이션을 호스팅하기 위해 Amazon Elastic Kubernetes Service(Amazon ECS)에 Kubernetes 클러스터를 구현했습니다. 이 회사는 다음 달에 애플리케이션 트래픽이 상당히 증가할 것으로 예상하고 많은 요청으로 인해 애플리케이션이 충돌하는 것을 방지하고자 합니다.
...
한 회사가 Amazon EC2 인스턴스에 새 애플리케이션을 배포합니다. 애플리케이션 코드는 AWS CodeCommit 리포지토리에 저장됩니다. 이 회사는 AWS CodePipeline 파이프라인을 사용하여 CI/CD(Continuous Integration and Continuous Delivery) 프로세스를 통해 EC2 인스턴스에 코드를 배포합니다.
...
SysOps 관리자가 기본 형식을 사용하여 VPC 흐름 로그를 구성했습니다. SysOps 관리자가 Amazon CloudWatch Logs를 대상으로 지정했습니다. 이 솔루션은 몇 달 동안 성공적으로 작동했습니다. 그러나 추가 문제 해결 요구 사항으로 인해 SysOps 관리자는 흐름 로그에 tcp-flags 필드를 포함해야 합니다.
...
SysOps 관리자가 Amazon CloudFront 배포의 캐시 적중률이 10% 미만임을 알아챘습니다. SysOps 관리자는 배포의 캐시 적중률을 높이고, 네트워크 성능을 개선하고, 오리진의 부하를 줄여야 합니다. SysOps 관리자는

이러한 요구 사항을 충족하기 위해 어떤 조치 조합을 취해야 합니까? (두 가지를 선택하세요.)
...
모든 Amazon EC2 인스턴스에 사용자 정의 애플리케이션을 설치해야 합니다. 애플리케이션은 작고, 자주 업데이트되며, 자동으로 설치할 수 있습니다.
...
한 회사가 AWS에서 전자상거래 애플리케이션을 실행하고 있습니다. 이 애플리케이션은 Amazon Aurora DB 클러스터에 열려 있지만 유휴 상태인 연결을 많이 유지합니다. 사용량이 가장 많은 시간에 데이터베이스는 다음과 같은 오류 메시지를 생성합니다. "연결이 너무 많습니다." 데이터베이스 클라이언트에도 오류가 발생합니다.
...
SysOps 관리자가 사용자 지정 네트워크 ACL을 활용하는 퍼블릭 및 프라이빗 서브넷이 있는 VPC의 문제를 해결하고 있습니다. 프라이빗 서브넷의 인스턴스는 인터넷에 액세스할 수 없습니다. 퍼블릭 서브넷에 연결된 인터넷 게이트웨이가 있습니다. 프라이빗 서브넷에는 퍼블릭 서브넷에 연결된 NAT 게이트웨이로 가는 경로가 있습니다. Amazon EC2 인스턴스는 VPC의 기본 보안 그룹과 연결되어 있습니다.
...
한 회사가 내부 데이터를 Amazon S3 버킷에 저장합니다. S3 버킷 내의 모든 기존 데이터는 Amazon S3 관리 암호화 키(SSE-S3)를 사용한 서버 측 암호화를 사용하여 보호됩니다. S3 버전 관리가 활성화되어 있습니다. SysOps 관리자는 재해 복구를 위해 내부 데이터를 다른 AWS 계정의 다른 S3 버킷으로 복제해야 합니다. 모든 기존 데이터는 소스 S3 버킷에서 대상 S3 버킷으로 복사됩니다.
...
한 회사에는 자동 확장 그룹의 10개 Amazon EC2 온디맨드 인스턴스로 구성된 플릿에 호스팅된 상태 없는 애플리케이션이 있습니다. 서비스 요구 사항을 충족하려면 최소 6개의 인스턴스가 필요합니다.
...
한 회사가 Auto Scaling 그룹에 있는 Linux 기반 Amazon EC2 인스턴스의 플릿에 배포되는 서비스를 개발했습니다. 이 서비스는 애플리케이션 코드의 오류로 인해 가끔 예기치 않게 실패합니다. 이 회사의 엔지니어링 팀은 서비스 실패의 근본 원인을 해결하는 데 몇 주가 걸릴 수 있다고 판단합니다.
...
회사의 웹 애플리케이션은 단일 AWS 지역의 Amazon EC2 인스턴스에서 실행됩니다. 인프라는 가용성 영역(AZ) 장애가 발생하더라도 성능 저하 없이 애플리케이션을 계속 사용할 수 있도록 설계해야 합니다. 최적의 성능을 보장하기 위해 애플리케이션은 항상 최소 12개의 인스턴스를 유지해야 합니다.
...
회사에서 Amazon Elastic Block Store(Amazon EBS) 볼륨의 디스크 사용률을 모니터링해야 합니다. EBS 볼륨은 Amazon EC2 Linux 인스턴스에 연결되어 있습니다. SysOps 관리자는 디스크 사용률이 80% 이상으로 증가하면 알림을 제공하는 Amazon CloudWatch 알람을 설정해야 합니다.
...
전자상거래 회사는 쇼핑 웹사이트에서 인기 있는 제품 쿼리의 메모리 내 캐싱을 위해 Amazon ElastiCache for Redis 클러스터를 사용합니다. 캐시 제거 정책은 TTL이 설정되었는지 여부에 관계없이 키를 무작위로 제거하는 것입니다. SysOps 관리자는 비용을 증가시키지 않고 캐시 적중률을 개선해야 합니다.
...
한 회사가 Application Load Balancer(ALB) 뒤의 여러 Amazon EC2 인스턴스에 새로운 웹 애플리케이션을 배포했습니다. EC2 인스턴스는 Auto Scaling 그룹에서 실행됩니다. 사용자들은 자주 로그인하라는 메시지가 표시된다고 보고합니다.
...
한 회사에는 Amazon S3 버킷에 대용량 데이터 객체를 업로드하는 과학자가 있습니다. 과학자들은 객체를 멀티파트 업로드로 업로드합니다. 멀티파트 업로드는 종종 최종 클라이언트 연결 상태가 좋지 않아 실패합니다.
...
한 회사가 세 개의 별도 AWS 리전에 프로비저닝된 Amazon S3 버킷에 데이터를 저장합니다. VPN을 사용하여 공용 인터넷을 통해 S3 버킷에서 데이터 센터로 데이터가 복사됩니다. SysOps 관리자는 가끔 전송이 평소보다 오래 걸리는 것을 알아차리고 문제가 회사 ISP 네트워크 내의 혼잡이라고 판단합니다. 관리자가
...
한 회사에 프로덕션 시스템을 지원하는 Amazon EC2 인스턴스가 있습니다. EC2 인스턴스는 Amazon Elastic Block Store(Amazon EBS) 볼륨으로 백업됩니다. EBS 볼륨의 드라이브가 100% 용량으로 채워져 EC2 인스턴스의 애플리케이션에 오류가 발생합니다.
...
한 회사에 AWS Organizations의 조직에 있는 여러 멤버 계정이 있습니다. 이 회사는 최근에 관리자가 계정 루트 사용자 자격 증명을 사용하고 있다는 사실을 발견했습니다. 이 회사는 관리자가 루트 사용자 자격 증명을 사용하여 Amazon EC2 인스턴스에서 어떤 작업도 수행하지 못하도록 해야 합니다.
...
한 회사가 Amazon EC2 인스턴스에서 호스팅되는 애플리케이션에서 벗어나고 있습니다. 이 회사는 Amazon S3, Amazon API Gateway, AWS Lambda, Amazon CloudFront를 사용하는 서버리스 아키텍처를 구현하려고 합니다. 이 전환의 일환으로 이 회사는 EC2 인스턴스가 종료된 후 모든 EC2 인스턴스와 연결되지 않은 Elastic IP 주소를 가지고 있습니다.
...
한 회사가 Amazon CloudFront를 사용하여 최종 사용자에게 정적 콘텐츠를 제공합니다. 이 회사의 마케팅 팀은 최근 회사 웹사이트에 150개 이미지에 대한 업데이트를 배포했습니다. 그러나 웹사이트에 일부 새 이미지가 표시되지 않습니다.
...
SysOps 관리자는 회사의 재해 복구 절차를 담당합니다. 이 회사는 프로덕션 계정에 소스 Amazon S3 버킷이 있으며, 소스에서 비프로덕션 계정의 대상 S3 버킷으로 객체를 복제하려고 합니다. SysOps 관리자는 S3 크로스 리전, 크로스 계정 복제를 구성하여 소스 S3 버킷을 대상 S3 버킷으로 복사합니다. SysOps 관리자가 대상 S3 버킷의 객체에 액세스하려고 하면 액세스 거부 오류가 발생합니다.
...
한 회사가 PostgreSQL Multi-AZ DB 클러스터용 Amazon RDS를 실행하고 있습니다. 이 회사는 AWS CloudFormation 템플릿을 사용하여 기본 크기가 100GB인 데이터베이스를 개별적으로 만듭니다. 이 회사는 매주 월요일에 데이터베이스를 만들고 매주 금요일에 데이터베이스를 삭제합니다.
...
SysOps 관리자가 프로덕션 데이터베이스 사본을 마이그레이션 계정과 공유하려고 합니다. 프로덕션 데이터베이스는 Amazon RDS DB 인스턴스에 호스팅되고 production-rds-key라는 별칭을 가진 AWS Key Management Service(AWS KMS) 키로 암호화됩니다. SysOps 관리자는 최소한의

관리 오버헤드로 이러한 요구 사항을 충족하기 위해 무엇을 해야 합니까?
...
한 회사가 애플리케이션 로드 밸런서 뒤에 있는 Amazon EC2 인스턴스 그룹에서 애플리케이션을 실행하고 있습니다. EC2 인스턴스는 3개의 가용성 영역에서 실행됩니다. 이 회사는 고객에게 애플리케이션에 대한 최대 2개의 정적 IP 주소를 제공해야 합니다.
...
한 회사가 Amazon S3 버킷에 민감한 재무 데이터를 저장하려고 합니다. 이 회사는 버킷에 대한 공개 읽기 또는 쓰기 액세스를 허용하지 않는 회사 정책을 가지고 있습니다. SysOps 관리자는 공개 읽기 또는 쓰기 액세스를 허용하는 S3 권한을 자동으로 제거하는 솔루션을 만들어야 합니다.
...
SysOps 관리자가 애플리케이션을 재구성하고 있습니다. SysOps 관리자는 데이터베이스가 퍼블릭 엔드포인트를 사용하는 퍼블릭 서브넷에서 퍼블릭 네트워크의 액세스를 제한하기 위해 프라이빗 서브넷으로 데이터베이스를 이동했습니다. 이 변경 후 데이터베이스에 대한 읽기 액세스가 필요한 AWS Lambda 함수는 데이터베이스에 연결할 수 없습니다. SysOps 관리자는 보안을 손상시키지 않고 이 문제를 해결해야 합니다.
...
사용자는 상태 저장 웹 애플리케이션에서 일관된 강제 로그아웃을 보고하고 있습니다. 로그아웃은 15분 애플리케이션 로그아웃 타이머가 만료되기 전에 발생합니다.
...
한 회사에는 연결된 Amazon Elastic Block Store(Amazon EBS) 볼륨에서 많은 파일을 읽고 쓰는 Linux Amazon EC2 Spot Instances 클러스터가 있습니다. EC2 인스턴스는 자주 시작되고 중지됩니다. EC2 인스턴스가 시작될 때 프로세스의 일부로 EBS 볼륨이 스냅샷에서 복원됩니다.
...
한 회사가 최근 프로덕션에 애플리케이션을 배포했습니다. 프로덕션 환경은 현재 애플리케이션의 웹 애플리케이션과 MariaDB 데이터베이스를 호스팅하는 단일 Amazon EC2 인스턴스에서 실행됩니다. 회사 정책에 따르면 모든 IT 프로덕션 환경은 고가용성이어야 합니다.
...
회사가 온프레미스와 AWS에서 워크로드를 실행하고 있습니다. SysOps 관리자는 AWS 서비스를 사용하여 온프레미스의 모든 서버에서 작업을 자동화해야 합니다. SysOps 관리자는 온프레미스 서버에 장기 자격 증명을 설치해서는 안 됩니다.
...
한 회사가 AWS에 애플리케이션을 배포했습니다. 이 애플리케이션은 Auto Scaling 그룹에 있는 Linux Amazon EC2 인스턴스 플릿에서 실행됩니다. Auto Scaling 그룹은 시작 템플릿을 사용하도록 구성되어 있습니다. 시작 템플릿은 기본 스토리지에 General Purpose SSD(gp3) EBS 볼륨을 사용하는 Amazon Elastic Block Store(Amazon EBS) 지원 EC2 인스턴스를 시작합니다.
...
한 회사에 AWS 계정 50개가 있고 각 계정에서 동일한 Amazon VPC를 만들고 싶어합니다. 회사가 앞으로 VPC에 적용하는 모든 변경 사항은 모든 VPC에 구현해야 합니다.
...
미디어 회사가 AWS에서 공개 뉴스 및 비디오 포털을 호스팅합니다. 이 포털은 프로비저닝된 용량이 있는 Amazon DynamoDB 테이블을 사용하여 Amazon S3 버킷에 저장된 비디오 파일의 인덱스를 유지합니다. 최근 이벤트 동안 수백만 명의 방문자가 뉴스를 보기 위해 포털을 방문했습니다. 트래픽이 증가하면서 DynamoDB 테이블에서 읽기 요청이 제한되었습니다. 포털에 비디오를 표시할 수 없었습니다.
...
한 회사가 Auto Scaling 그룹의 일부인 여러 Amazon EC2 인스턴스에서 웹 애플리케이션을 실행합니다. 이 회사는 인스턴스의 CPU 사용률이 50%를 넘으면 Auto Scaling 그룹이 확장되기를 원합니다.
...
회사의 VPC에 기존 IPv4 구성이 있습니다. IPv4 구성에는 퍼블릭 서브넷, 프라이빗 서브넷, NAT 게이트웨이, 기본 경로 테이블 및 ACL이 포함됩니다.
...
한 회사가 세 개의 Amazon EC2 인스턴스에서 작업자 프로세스를 실행합니다. 인스턴스는 간단한 확장 정책을 사용하도록 구성된 자동 확장 그룹에 있습니다. 인스턴스는 Amazon Simple Queue Service(Amazon SQS) 대기열의 메시지를 처리합니다.
...
한 회사가 VPC의 퍼블릭 서브넷에 NAT 게이트웨이를 만들었습니다. VPC에는 Amazon EC2 인스턴스를 포함하는 프라이빗 서브넷도 있습니다. EC2 인스턴스는 NAT 게이트웨이를 사용하여 인터넷에 액세스하여 패치와 업데이트를 다운로드합니다. 이 회사는 NAT 게이트웨이의 탄력적 네트워크 인터페이스에 대한 VPC 흐름 로그를 구성했습니다. 이 회사는 출력을 Amazon CloudWatch Logs에 게시하고 있습니다.
...
한 회사가 여러 AWS 계정에서 프로덕션 애플리케이션을 관리합니다. 이 회사는 Amazon Linux 2를 실행하는 Amazon EC2 인스턴스에서 프로덕션 애플리케이션을 호스팅합니다. EC2 인스턴스는 여러 VPC에 분산되어 있습니다. 각 VPC는 ​​프라이빗 DNS에 자체 Amazon Route 53 프라이빗 호스팅 영역을 사용합니다.
...
회사에 온프레미스 DNS 솔루션이 있으며 example.com의 Amazon Route 53 프라이빗 호스팅 영역에서 DNS 레코드를 확인하려고 합니다. 이 회사는 온프레미스 네트워크와 VPC 간의 네트워크 연결을 위해 AWS Direct Connect 연결을 설정했습니다. SysOps 관리자는 온프레미스 서버가 example.com 도메인의 레코드를 쿼리할 수 있는지 확인해야 합니다.
...
SysOps 관리자는 Linux Amazon EC2 인스턴스에서 실행되는 프로세스를 모니터링해야 합니다. 프로세스가 중지되면 프로세스가 자동으로 다시 시작되어야 합니다. Amazon CloudWatch 에이전트는 모든 EC2 인스턴스에 이미 설치되어 있습니다.
...
한 회사에 MariaDB Multi-AZ 데이터베이스를 위한 Amazon RDS를 사용하는 애플리케이션이 있습니다. 계획된 유지 관리 이벤트 중에 데이터베이스가 장애 조치를 경험할 때마다 애플리케이션이 몇 분 동안 사용할 수 없게 됩니다.
...
글로벌 기업이 전 세계 누구나 ​​모바일 폰에서 비디오를 업로드할 수 있도록 허용하고자 합니다. 이 회사의 모바일 앱은 추가 처리를 위해 공개 인터넷을 통해 비디오를 us-east-1 지역의 Amazon S3 버킷에 업로드합니다. us-east
...
SysOps 관리자가 AWS Lambda 함수를 호출하는 Amazon EventBridge 규칙을 프로비저닝하는 AWS CloudFormation 템플릿을 생성했습니다. Lambda 함수는 Amazon CloudWatch 로그 그룹에 이벤트 세부 정보를 쓰도록 설계되었습니다. 이 함수는 Amazon CloudWatch Logs에 이벤트를 쓸 수 있는 권한이 있습니다. 그러나 SysOps 관리자는 Lambda 함수가 실행 중이 아니라는 것을 발견했습니다.
...
한 회사에 CPU 사용률이 높은 Amazon EC2 인스턴스가 있습니다. EC2 인스턴스는 t3.large 인스턴스이며 테스트 웹 애플리케이션을 실행하고 있습니다. 이 회사는 웹 애플리케이션이 컴퓨팅 최적화된 대형 인스턴스에서 더 잘 작동할 것이라는 것을 알게 되었습니다.
...
개발팀이 15분 전에 새로운 AWS Lambda 함수를 생성하고 배포했습니다. 함수가 여러 번 호출되었지만 Amazon CloudWatch Logs에는 로그 메시지가 표시되지 않습니다.
...
한 회사에서 새로 만든 Amazon CloudWatch 알람이 INSUFFICIENT_DATA 상태에서 전환되지 않는 것을 관찰했습니다. 이 알람은 퍼블릭 서브넷에 배포된 Amazon EC2 인스턴스의 mem_used_percent 메트릭을 추적하기 위해 만들어졌습니다.
...
AWS Organizations를 사용하는 한 회사가 최근 AWS Control Towerю를 구현했습니다. 이 회사는 이제 ID 관리를 중앙화해야 합니다. SysOps 관리자는 AWS ШAM Identity Center를 외부 SAML 2.0 ID 공급자(IdP)와 페더레이션하여 회사의 모든 계정과 클라우드 애플리케이션에 대한 액세스를 중앙에서 관리해야 합니다.
...
한 회사가 AWS 클라우드에서 다중 계정 구조를 사용합니다. 이 회사의 환경에는 공통 리소스를 위한 공유 계정이 포함됩니다. 이 환경에는 새로운 애플리케이션 개발을 위한 개발 계정도 포함됩니다. 이 회사는 DNS 관리를 위해 Amazon Route 53을 사용합니다. 이 회사는 공유 계정에서 모든 Route 53 호스팅 영역을 관리합니다.
