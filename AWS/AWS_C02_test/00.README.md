## AWS API Gateway 스테이지 Lamda 함수 alias 설정
...
# CI/CD
## copilot, codePipeline,CodeBuild,CodeCommit

AWS에서 CI/CD 도구들은 각각 특정 기능과 사용 사례에 맞춰 설계되어 있습니다. 아래는 AWS Copilot, AWS CodePipeline, AWS CodeBuild, AWS CodeCommit의 비교와 설명입니다.

### 1. **AWS Copilot**
- **목적**: AWS Copilot은 컨테이너화된 애플리케이션의 배포를 간소화하기 위한 CLI 도구입니다.
- **주요 기능**:
  - **애플리케이션 생성 및 관리**: 애플리케이션, 서비스, 환경(예: 개발, 스테이징, 프로덕션)을 쉽게 생성하고 관리할 수 있습니다.
  - **CI/CD 설정**: 기본적으로 CI/CD 파이프라인을 자동으로 생성하여 배포할 수 있도록 지원합니다.
  - **서버리스 및 ECS 통합**: AWS Fargate 및 Amazon ECS에 최적화되어 있으며, 서버리스 애플리케이션도 지원합니다.
- **사용 예시**: 컨테이너화된 웹 애플리케이션을 구축하고, 손쉽게 다양한 환경에 배포할 때 사용됩니다.

### 2. **AWS CodePipeline**
- **목적**: AWS CodePipeline은 지속적인 통합 및 지속적인 배포(CI/CD) 서비스입니다.
- **주요 기능**:
  - **파이프라인 정의**: 소스 코드, 빌드, 테스트, 배포 단계를 포함하는 복잡한 파이프라인을 정의할 수 있습니다.
  - **다양한 서비스 통합**: AWS CodeBuild, AWS Lambda, ECS, S3 등과 통합하여 자동화된 배포 프로세스를 생성할 수 있습니다.
  - **조건부 논리**: 조건부 실행 및 다양한 트리거를 설정할 수 있습니다.
- **사용 예시**: 여러 서비스와 도구를 연결하여 복잡한 CI/CD 워크플로를 관리할 때 사용됩니다.

### 3. **AWS CodeBuild**
- **목적**: AWS CodeBuild는 빌드 프로세스를 자동화하는 서비스입니다.
- **주요 기능**:
  - **빌드 환경 제공**: 다양한 프로그래밍 언어와 빌드 도구를 지원하며, Docker 컨테이너에서 빌드 환경을 정의할 수 있습니다.
  - **자동 스케일링**: 빌드 요구에 따라 자동으로 스케일링되어 빠르게 빌드를 완료할 수 있습니다.
  - **통합**: CodePipeline과 통합되어 빌드 단계를 자동으로 수행할 수 있습니다.
- **사용 예시**: 애플리케이션 소스 코드를 빌드하고 테스트하여 배포 준비가 완료된 아티팩트를 생성할 때 사용됩니다.

### 4. **AWS CodeCommit**
- **목적**: AWS CodeCommit은 Git 기반의 소스 코드 저장소입니다.
- **주요 기능**:
  - **완전 관리형**: Git 저장소를 완전 관리형으로 제공하여, 코드 저장, 버전 관리 및 협업을 지원합니다.
  - **보안**: IAM을 사용하여 접근 제어를 설정할 수 있으며, HTTPS와 SSH를 통한 보안 액세스를 지원합니다.
  - **통합**: CodePipeline 및 CodeBuild와 통합하여 CI/CD 프로세스의 소스 단계로 사용됩니다.
- **사용 예시**: 소스 코드를 안전하게 저장하고, 팀원 간의 협업을 지원하는 저장소로 사용됩니다.

### 요약
- **AWS Copilot**: 컨테이너화된 애플리케이션을 쉽게 관리하고 CI/CD를 설정할 수 있는 도구.
- **AWS CodePipeline**: 다양한 서비스와 통합하여 복잡한 CI/CD 파이프라인을 정의할 수 있는 서비스.
- **AWS CodeBuild**: 자동으로 빌드 프로세스를 수행하는 서비스.
- **AWS CodeCommit**: Git 기반의 소스 코드 저장소로, 안전한 코드 관리를 지원.

이러한 도구들은 각각의 역할을 가지고 있으며, 필요에 따라 함께 사용하여 강력한 CI/CD 파이프라인을 구축할 수 있습니다.


## CI/CD 도구

AWS에서 CI/CD(지속적 통합 및 지속적 배포)를 구현하기 위한 도구로는 여러 가지가 있습니다. 주요 도구와 그 사용 예시는 다음과 같습니다:

### 1. **AWS CodePipeline**
- **설명**: AWS CodePipeline은 CI/CD 서비스를 제공하는 완전관리형 도구로, 애플리케이션의 빌드, 테스트, 배포를 자동화할 수 있습니다.
- **사용 예시**:
  - GitHub에 있는 코드 저장소에서 소스 코드를 가져와 AWS CodeBuild로 빌드하고, 테스트 후 Amazon ECS에 배포하는 파이프라인을 구성합니다.

### 2. **AWS CodeBuild**
- **설명**: AWS CodeBuild는 소스 코드를 컴파일하고 테스트하여 배포 가능한 아티팩트를 생성하는 완전관리형 빌드 서비스입니다.
- **사용 예시**:
  - Java 애플리케이션을 Maven으로 빌드하고, Docker 이미지를 생성하여 Amazon ECR에 푸시합니다.

### 3. **AWS CodeDeploy**
- **설명**: AWS CodeDeploy는 애플리케이션을 EC2 인스턴스, Lambda 함수 또는 온프레미스 서버에 자동으로 배포하는 서비스입니다.
- **사용 예시**:
  - 새로운 Docker 이미지를 Amazon ECS 클러스터에 배포하거나, EC2 인스턴스에 애플리케이션 코드를 업데이트하는 데 사용할 수 있습니다.

### 4. **AWS CodeCommit**
- **설명**: AWS CodeCommit은 완전관리형 소스 제어 서비스로, Git 저장소를 호스팅할 수 있습니다.
- **사용 예시**:
  - 개발자들이 애플리케이션 코드를 CodeCommit 리포지토리에 푸시하고, CodePipeline이 이를 감지하여 자동으로 빌드 및 배포 과정을 시작하도록 설정합니다.

### 5. **AWS CloudFormation**
- **설명**: AWS CloudFormation은 인프라를 코드로 정의할 수 있게 해주는 서비스로, 스택을 통해 리소스를 배포하고 관리합니다.
- **사용 예시**:
  - CI/CD 파이프라인의 일환으로, 애플리케이션 배포 시 필요한 AWS 리소스를 정의하는 템플릿을 사용하여 자동으로 인프라를 설정합니다.

### 6. **Amazon EKS (Elastic Kubernetes Service)**
- **설명**: Amazon EKS는 Kubernetes를 관리하는 서비스로, 컨테이너화된 애플리케이션의 배포, 관리 및 확장을 지원합니다.
- **사용 예시**:
  - CI/CD 파이프라인에서 Docker 이미지를 빌드하고 EKS 클러스터에 배포하여 마이크로서비스 아키텍처를 구현합니다.

### 7. **AWS Lambda**
- **설명**: AWS Lambda는 서버리스 컴퓨팅 서비스로, 코드를 자동으로 실행할 수 있습니다.
- **사용 예시**:
  - 코드가 GitHub에 푸시될 때마다 Lambda 함수가 트리거되어 CodeBuild를 호출하고, 빌드가 완료된 후 CodeDeploy로 배포합니다.

이러한 도구들을 조합하여, AWS에서 CI/CD 파이프라인을 구축함으로써 애플리케이션의 개발 및 배포 과정에서의 자동화와 효율성을 높일 수 있습니다.
...
AWS에서 **장기 자격 증명(long-term credentials)**은 사용자가 AWS 리소스에 액세스하기 위해 지속적으로 사용하는 고정된 자격 증명입니다. 일반적으로 다음과 같은 두 가지 형태로 존재합니다:

1. **AWS Access Key ID와 Secret Access Key**:
   - 이들은 AWS 계정이나 IAM 사용자에게 발급되는 고유한 자격 증명입니다. 이 자격 증명을 사용하여 API 호출을 통해 AWS 서비스에 접근할 수 있습니다. 
   - 이러한 자격 증명은 안전하게 관리해야 하며, 노출되면 보안 위험이 발생할 수 있습니다.

2. **IAM 사용자 비밀번호**:
   - AWS 관리 콘솔에 로그인할 때 사용하는 비밀번호로, 이 역시 장기 자격 증명으로 간주됩니다. 
   - 비밀번호는 일반적으로 변경하지 않거나 오랜 기간 동안 사용됩니다.

### 단점
- 장기 자격 증명을 사용할 경우, 자격 증명이 노출될 경우 보안 문제가 발생할 수 있으며, 관리가 복잡해질 수 있습니다.
- 자격 증명을 주기적으로 교체해야 하며, 사용자에게 적절한 권한만 부여해야 합니다.

### 대안
AWS는 장기 자격 증명 대신 **임시 자격 증명**(temporary credentials)을 사용할 것을 권장합니다. 예를 들어:

- **IAM Roles**: AWS 서비스나 EC2 인스턴스에 IAM 역할을 부여하여 필요한 자격 증명을 임시로 제공할 수 있습니다. 이 방식은 안전하고 자동으로 자격 증명을 관리합니다.
- **AWS STS (Security Token Service)**: IAM 역할을 사용하여 임시 자격 증명을 요청할 수 있으며, 이를 통해 애플리케이션이나 서비스가 특정 권한으로 AWS 리소스에 액세스할 수 있습니다.

이런 방식은 보안성을 높이고, 자격 증명의 유출 위험을 줄여줍니다.
